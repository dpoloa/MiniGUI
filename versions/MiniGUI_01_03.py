...
class HostDialog(BaseDialog):
    ...
    def setEthernetIntfs(self):
        ...
        # Top layout for IP address and netmask
        widget_top = QWidget()
        layout_top = QHBoxLayout()
        widget_top.setLayout(layout_top)
        layout.addWidget(widget_top)

        # Design for top layout
        layout_top.addWidget(QLabel("IP Address / Netmask:"))

        # IP address
        ip_edit_label = QLineEdit(str(self.properties["IP"]))
        self.results["IP"] = ip_edit_label
        layout_top.addWidget(ip_edit_label)
        ...
        layout_top.addWidget(mask_edit_label)

        # Bottom layout for Ethernet interfaces
        eth_group = QGroupBox("Interface state")
        ...
        for interface in host_intfs:
            ...
            eth_layout.addWidget(intf_state_button, index + 1, 2, Qt.AlignLeft)
    ...
    def setRoutingTable(self):
        ...
        if not scene.net_running:
            layout.addWidget(QLabel("Routing table is not available as net is not running"))
            layout.addStretch()
            return widget

        # Obtenemos todas las rutas almacenadas por el usuario
        route_list = self.host.net_controller.getRoutingTable(self.host)
        if route_list == "Error":
            layout.addWidget(QLabel("An error occurred getting the routing table. Please, restart the dialog"))
            layout.addStretch()
            return widget

        layout.addWidget(QLabel("Write here to add a new route:"))

        # Layout para añadir rutas usando comandos
        widget_top = QWidget()
        layout_top = QHBoxLayout()
        ...
        # Widget (y layout) contenedor de las rutas obtenidas
        route_widget = QWidget()
        layout.addWidget(route_widget)

        # En el caso de no haya habido ningún problema obteniendo la tabla de rutas, la mostramos al usuario
        ...
        if not route_list["Destination"]:
            # En el caso de que no haya ninguna ruta almacenada, mostramos un elemento vacío
            route_layout.addWidget(QLabel("There are no entries yet"), 0, 0, 1, -1, Qt.AlignHCenter)
        else:
            # En caso contrario, las mostramos al usuario
            ...
            for index in range(len(route_list["Destination"])):
                ...
                delete_command = ("ip route del " + str(route_list["Destination"][index]) +
                                  " dev " + route_list["Interface"][index])
                delete_button = QPushButton("Delete")
                delete_button.clicked.connect(lambda: self.sendCommandToNet(delete_command, route_widget))
                route_layout.addWidget(delete_button, index + 1, 6)

        # Añadimos acción al botón push del primer layout
        send_button.clicked.connect(lambda: self.sendCommandToNet(line_command.text(), route_widget))
    ...
    def sendCommandToNet(self, command, widget):
        ...
        if output is not None:
            ...
            dialog.setInformativeText("The command you inserted was not recognized. See details for more information")
    ...
    def updateRoutingTableLayout(self, route_widget, route_list):
        ...
        # Obtenemos el layout del widget dinámico y lo vaciamos
        ...
        # Una vez vaciado lo llenamos con las nuevas rutas
        if not route_list["Destination"]:
            route_layout.addWidget(QLabel("There are no entries yet"), 0, 0, 1, -1, Qt.AlignHCenter)
        else:
            route_layout.addWidget(QLabel("Destination"), 0, 0)
...
class RouterDialog(BaseDialog):
    ...
    def setRoutingTable(self):
        ...
        # Checking if scene is available. If not, data cannot be retrived
...
class NodeGUI(QGraphicsPixmapItem):
    ...
    def nodePropertiesDialog(self):
        ...
        if dialog.exec() and self.node_type != "Switch":
            ...
            # According to the node's type, there are two different ways.
...
class MiniGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.app_prefs = {"Mode": "basic", "CLI": True}
    ...
    def updateRoutingTable(self, node, command):
        """Function to send an update command to node and get its output"""
    ...
    def getRoutingTable(self, node=None):
        """Function used to retrieve the routing table for hosts and routers"""
        if self.net is None or not isinstance(node, NodeGUI):
            return
    ...
    def changePreferences(self, preference=None):
        """Function to change some preference"""
        if preference == "theme":
            global app_theme
            if app_theme == "light":
                app_theme = "dark"
...
def imagesMiniGUI():
    ...
    if app_theme == "light":
        return {
            "Host": "./images_01_03/laptop.png",
            "Switch": "./images_01_03/switch.png",
            "Router": "./images_01_03/router.png",
            "Link": "./images_01_03/cable.png",
            "Select": "./images_01_03/select.png",
            "Delete": "./images_01_03/delete.png"
        }
    else:
        return {
            "Host": "./images_01_03/laptop.png",
            "Switch": "./images_01_03/switch.png",
            "Router": "./images_01_03/router.png",
            "Link": "./images_01_03/cable_white.png",
            "Select": "./images_01_03/select_white.png",
            "Delete": "./images_01_03/delete_white.png"
        }
